...
namespace Bannerlord.SaveSystem.Patches
{
    public static class FieldLoadDataPatch
    {
        public static HarmonyPatchEntry FillObject_SetNonNullValues { get; } = new HarmonyPatchEntry(
            AccessTools.DeclaredMethod(Type.GetType("TaleWorlds.SaveSystem.Load.FieldLoadData, TaleWorlds.SaveSystem"), "FillObject"),
            new HarmonyMethod(typeof(FieldLoadDataPatch), nameof(FillObjectFinalizer)),
            HarmonyPatchType.Finalizer);


        private static MethodInfo IsContainerMethod { get; } = AccessTools.DeclaredMethod(Type.GetType("TaleWorlds.SaveSystem.TypeExtensions, TaleWorlds.SaveSystem"), "IsContainer", new[] { typeof(Type), typeof(TSS.ContainerType).MakeByRefType() });
        private static MethodInfo GetDataToUseMethod { get; } = AccessTools.DeclaredMethod(Type.GetType("TaleWorlds.SaveSystem.Load.VariableLoadData, TaleWorlds.SaveSystem"), "GetDataToUse");
